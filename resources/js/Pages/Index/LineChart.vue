...
export default {
  ...
  methods: {
    fetchData(matchFormat) {
      axios.get(`/top-bowling-players?query=${matchFormat}`)
        .then(response => {
          this.bowlingData = response.data;
          this.renderChart();
        })
        .catch(error => {
    ...
    renderChart() {
  const playerNames = this.bowlingData.map(entry => entry.label);
  const runs = this.bowlingData.map(entry => entry.value);
...
export default {
  ...
  methods: {
    fetchChartData() {
      axios.get("/top-bowling-players").then((response) => {
        this.chartData = {
          labels: response.data.map((item) => item.label),
          datasets: [
            {
              label: "Top 10 bowling Players",
...
<!-- select the top 10 bowling players as the label of the chart and the runs as the value in Laravel from the database tables (bowling and players), where players table has the foreign key player_id in bowling table. create a chart using vue-chartjs in a Laravel Vue application  -->
  ...
  methods: {
    async fetchChartData() {
      try {
       ...
       await axios.get("/top-bowling-players").then((response) => {
      this.localChartData = {
        labels: response.data.map((item) => item.label),
        datasets: [
          {
